...
export default class Highscore extends Phaser.GameObjects.Container {
    // private url: string = '';
    static url: string= `https://api-hidanz.herokuapp.com/user/`;
    private bg :Phaser.GameObjects.Image
    private widthBg :number

    constructor(scene: Phaser.Scene, x: number, y: number,username?:string|null,score?:number) {
        super(scene, x, y);
         this.bg = this.scene.add.image(0,0, 'leader').setDepth(4).setOrigin(-0.16,0.12).setScale(0.4)
        ...
        result.then((result: any[]) => {
            ...
            for (let i = 0; i <= 9; i += 1) {
                ...
                if (i >= 1) {
                    rank.y = prevRank.y + 20;
                    name1.y = prevName.y + 20;
                    scoreN.y = prevScore.y + 20;
                }
